...
namespace ASPax.Editor
{
    ...
    public class ProgressBarPropertyDrawer : PropertyDrawerBase
    {
        protected override float GetPropertyHeight_Internal(SerializedProperty property, GUIContent label)
        {
            ...
            return IsNumber(property) && IsNumber(maxValue) ? GetPropertyHeight(property) : GetPropertyHeight(property) + GetHelpBoxHeight();
        }
        ...
        protected override void OnGUI_Internal(Rect rect, SerializedProperty property, GUIContent label)
        {
            ...
            {
                var message = string.Format( "The provided dynamic max value for the progress bar is not correct. Please check if the '{0}' is correct, or the return type is float/int", nameof(progressBarAttribute.MaxValueName));
                DrawDefaultPropertyAndHelpBox(rect, property, message, MessageType.Warning);
            }
        ...
        private object GetMaxValue(SerializedProperty property, ProgressBarAttribute progressBarAttribute)
        {
            ...
            else
            {
                ...
                if (methodValuesInfo != null && (methodValuesInfo.ReturnType == typeof(float) || methodValuesInfo.ReturnType == typeof(int)) && methodValuesInfo.GetParameters().Length == 0)
        ...
        private void DrawBar(Rect rect, float fillPercent, string label, Color barColor, Color labelColor)
        {
            ...
            var align = GUI.skin.label.alignment; // set alignment and cache the default
            GUI.skin.label.alignment = TextAnchor.UpperCenter;

            var c = GUI.contentColor; // set the color and cache the default
            GUI.contentColor = labelColor;

            var labelRect = new Rect(rect.x, rect.y - 2, rect.width, rect.height); // calculate the position

            EditorGUI.DropShadowLabel(labelRect, label); // draw~

            GUI.contentColor = c; // reset color and alignment
